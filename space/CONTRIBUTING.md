...
- `Space` (class)
- `Space.open(name, opts)`
- `Space.join(name, opts)`
- `Space.prototype.post(message)`
- `Space.prototype.messages(opts)`
- `Space.prototype.leave()`
- `Space.prototype.end()`
- `Presence` (class) and its `register` / `heartbeat` / `list` methods
- `Notification` (class) and its `send` / `list` / `ack` methods

### Explicitly unstable until v1.0

Even after v0.1.0:

- On-disk file layout (the *contents* are stable; the *paths* under the space root may move)
- SQLite schema for live state (live state is wipeable by design)
- HTTP route paths (the *behavior* is stable; the *routing* may be reorganized)
- Notification TTL defaults per source kind
- Internal cursor / ordering implementation
...
- Changing the on-disk *content* format (the paths are unstable; the contents are not)
...
Per the CHARTER, work proceeds in slices. Rules:
...
- Integration tests that hit the filesystem use temp directories under `os.tmpdir()` and clean up in `afterEach`.
- Integration tests that spin up the HTTP server are tagged `// @integration` and skipped by default. Run with `npm run test:integration` (added when the first integration test lands).
...
- No legacy memo code imports. Algorithms are re-implemented against the CHARTER; references in comments are fine.
...
This rule applies especially to the daily smoke test defined in `CHARTER.md`. If the smoke test fails after a slice ships, that slice does not ship — it gets reverted or fixed, not waived.
